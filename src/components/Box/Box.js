...
function Box(props) {
  ...
  const onSubmitDelete = async (id) => {
    const response = await fetch(
      `https://online-energy-utility-plat.herokuapp.com/api/admin/user/${props.product.userId}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
  ...
  const onSubmitMapping = async (event) => {
    ...
    const response = await fetch(
      `https://online-energy-utility-plat.herokuapp.com/api/admin/user-device`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
